    ...
    public class StartUp
    {
        static void Main(string[] args)
        {
            ...
            while ((command = Console.ReadLine()) != "End")
            {
                ...
                if (tokens.Length == 2)
                {
                    if (!tree.Any(p => p.Bithdate == tokens[0]))
                    {
                        tree.Add(new Person(tokens[0]));
                    }

                    var parent = tree.First(p => p.Bithdate == tokens[0]);

                    if (!tree.Any(p => p.Bithdate == tokens[1]))
                    {
                        tree.Add(new Person(tokens[1]));
                    }

                    var child = tree.First(p => p.Bithdate == tokens[1]);
                ...
                else if (tokens.Length == 3)
                {
                    if (command.Contains("-"))
                    {
                        ...
                        {
                            if (!tree.Any(p => p.Bithdate == tokens[2]))
                            {
                                tree.Add(new Person(tokens[2]));
                            }

                            var child = tree.First(p => p.Bithdate == tokens[2]);
                    ...
                    {
                        var persons = tree.Where(p => (p.FirstName == tokens[0] && p.LastName == tokens[1]) || p.Bithdate == tokens[2]);
            ...
            if (manInfo.Length == 1)
            {
                target = tree.First(p => p.Bithdate == manInfo[0]);
            }
            else if (manInfo.Length == 2)
            {
